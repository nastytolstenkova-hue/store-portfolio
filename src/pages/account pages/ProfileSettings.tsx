    ...
    <div className="border border-zinc-400 bg-amber-100/20 rounded-md py-2 px-3 cursor-default">
      ...
      <div className="border border-zinc-400 rounded-md p-2 m-2">
        <label className="flex justify-between my-2">
          ...
          <input name='userName' disabled={!isEdit} className={inputDesign} placeholder="Write User Name here" type='text' value={user.userName} onChange={handleChange}/>
        </label>
        <label className="flex justify-between my-2 ">
          <p className="line-clamp-1 text-nowrap text-sm">Email Address</p>
          <input name='email' disabled={!isEdit} className={inputDesign} placeholder="Write email here" type='email' value={user.email} onChange={handleChange}/>
        </label>
       ...
       <div className="border border-zinc-400 rounded-md p-2 m-2">
        ...
        <label className="flex justify-between my-2">
          ...
          <input name='phone' disabled={!isEdit} className={inputDesign} type='text' placeholder="Write your phone" value={user.phone} onChange={handleChange}/>
        </label>
        <label className="flex justify-between my-2">
          <p className="line-clamp-1 text-nowrap text-sm">Date of Birth</p>
          <input name='birthDate' className={inputDesign} type='date' placeholder="Date of Birth" disabled={!isEdit} value={user.birthDate} onChange={handleChange}/>
        </label>
        <label className="flex justify-between my-2">
          <p className="line-clamp-1 text-nowrap text-sm">Preferred Contact Method</p>
          <select disabled={!isEdit} name='contactMethod' id='contactMethod' value={user.contactMethod} onChange={handleChange}>
      ...
      <div className="border border-zinc-400 rounded-md p-2 m-2">
        ...
        <label className="flex justify-between my-2">
          ...
          <input disabled={!isEdit} className={inputDesign} type='password' value={inputPassw.curPas} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setInputPassw((prevVal)=> ({ ...prevVal, curPas: e.target.value }))}/>
        </label>
        <label className="flex justify-between my-2">
          <p className="line-clamp-1 text-nowrap text-sm">New Password</p>
          <input disabled={!isEdit} className={inputDesign} type='password' value={inputPassw.newPas} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setInputPassw((prevVal)=> ({ ...prevVal, newPas: e.target.value }))}/>
        </label>
        <label className="flex justify-between my-2">
          <p className="line-clamp-1 text-nowrap text-sm">Confirm New Password</p>
          <input disabled={!isEdit} className={inputDesign} type='password' value={inputPassw.repeatNewPas} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setInputPassw((prevVal)=> ({ ...prevVal, repeatNewPas: e.target.value }))}/>
        </label>
        {passError && (
          <p className={`${passError === 'Success!' ? 'text-green-600' : 'text-red-500'} text-xs mt-1`}>
        {passError}
          </p>
        )}
        <button className="flex justify-center items-center mx-auto my-3 mb-2 px-2 py-0.5 uppercase border text-white border-zinc-300 bg-zinc-400/30 rounded-xl cursor-pointer whitespace-nowrap hover:bg-zinc-500/40  transition-colors duration-300 active:scale-95 " onClick={() => isChangePassw ? handleChangePassw() : setIsChangePassw(true)}>{!isChangePassw ? 'Change Password' : 'Save'}</button>
      </div>
      <div className="flex justify-between my-2 m-2 ">
        <button type="button" onClick={handleSave} className="flex justify-center items-center mx-auto my-3 w-[60%] mb-2 p-1 uppercase border border-zinc-300 shadow-[0_0_10px_2px_rgba(255,180,0,0.5)] bg-yellow-500/20 rounded-xl cursor-pointer whitespace-nowrap hover:bg-yellow-500/30  transition-colors duration-300 active:scale-95 ">{isEdit ? 'Save' : 'Change information'}</button>
