...
export default function ProductPage(){
  ...
  const {addCartProduct, productsList, addWishList } = UseProductContext();
      ...
      <div>
        ...
        <div className="grid grid-cols-[1fr_2fr] ml-5 my-5">
          <div className="flex border border-zinc-300 w-fit h-fit rounded-md ">
            <button className={`${activeBatton} ${buttonDes}`} onClick={()=>setQuality(prevVal=>prevVal - 1)}>-</button>
            
            <p className="flex justify-center items-center p-1 w-7 h-7">{quality}</p>
            <button className={`${activeBatton} ${buttonDes}`} onClick={()=>setQuality(prevVal=>prevVal + 1)}>+</button>
            
          </div>
          <div>
            <button className="flex justify-center items-center mx-auto p-1 uppercase   bg-yellow-300/30   rounded-xl w-4/5 cursor-pointer  shadow-[0_0_10px_2px_rgba(255,180,0,0.5)] whitespace-nowrap hover:bg-yellow-300/50 hover:border-white transition-colors duration-300 active:scale-95 " onClick={()=>{addCartProduct(product.id, quality);setQuality(1)}}>add to cart</button>
          </div>
        </div>
        <button className="cursor-pointer  active:scale-95" onClick={()=>addWishList(product.id)}>🤍 <span className="underline text-base text-zinc-600 hover:text-black  transition-colors duration-300 ">add to wishlist</span></button>
