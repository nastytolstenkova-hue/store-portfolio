...
export default function OneWishListProd({product}:{product:IProduct}){
  ...
  return (
    <div className="grid grid-cols-[1fr_2fr] border border-zinc-500/50 h-fit rounded-md p-2 mb-2 ">
      <NavLink to={`/catalog/${product.id}`} className="w-30 aspect-4/5 overflow-hidden rounded-md transition-transform duration-500 hover:scale-105">
        <img src={product.image} alt={product.name} className="w-full h-full object-cover  bg-zinc-100  "/>
      </NavLink>
      <div >
      ...
      >
        ...
        <div className="flex justify-between mx-2 mt-3 text-base ">
          ...
          <div className="">
            <button className="flex justify-center items-center mx-auto my-3 mb-2 p-1 uppercase border border-zinc-300 shadow-[0_0_10px_2px_rgba(255,180,0,0.5)] bg-zinc-300/30 rounded-xl cursor-pointer whitespace-nowrap hover:bg-yellow-200/20  transition-colors duration-300 active:scale-95 " onClick={()=>addCartProduct(product.id)}>Add to cart</button>
          </div>
